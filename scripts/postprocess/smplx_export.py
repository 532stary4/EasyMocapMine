...
def main(params):
    ...
    for pid, data in motions.items():
        ...
        if (params['bvh']):
            ...
            bpy.ops.export_anim.bvh(filepath=join(params['out'], 'SMPLX_{}.bvh'.format(pid)), frame_start=0, frame_end=nFrames-1)
        else:
            bpy.context.view_layer.objects.active = obj
            bpy.ops.object.smplx_export_unity_fbx(filepath=join(params['out'], 'SMPLX_{}.fbx'.format(pid)), check_existing=False)
